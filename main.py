...
pygame.display.set_caption("Space Killa")
...
class Enemy(Ship):
    ...
    def __init__(self, x, y, color, health=100):
        super().__init__(x, y, health)
        self.laser_img, self.ship_img = self.COLOR_MAP[color]
        self.mask = pygame.mask.from_surface(self.ship_img)
...
def main():
    ...
    def redraw_window():
        ...
        # blit draws it to the location given and win is short for window defined up above
        WIN.blit(BG, (0, 0))
        # draw text
    ...
    while run:
        ...
        # this is the exit button top right
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
        # movement of you're ship
...
def main_menu():
    title_font = pygame.font.SysFont('comiconsans', 70)
    run = True
    while run:
        WIN.blit(BG, (0, 0))
